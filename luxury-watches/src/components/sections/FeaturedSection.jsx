import React from 'react'
import { products, formatPrice } from '../../config/products'
import { shop } from '../../config/shop'

export default function FeaturedSection({ onEnquire }) {
  const featured = products.find((p) => p.featured) || products[0]

  return (
    <section className="featured-section">
      <div className="section-label">Spotlight</div>
      <div className="featured-grid">
        <div className="featured-visual">
          <svg width="200" height="200" viewBox="0 0 100 100" fill="none" xmlns="http://www.w3.org/2000/svg">
            <circle cx="50" cy="50" r="42" stroke="#c9a96e" strokeWidth="1.5" />
            <circle cx="50" cy="50" r="38" stroke="#333" strokeWidth="0.5" />
            <circle cx="50" cy="50" r="35" stroke="#555" strokeWidth="0.3" />
            <line x1="50" y1="50" x2="50" y2="18" stroke="#c9a96e" strokeWidth="2.5" strokeLinecap="round" />
            <line x1="50" y1="50" x2="68" y2="38" stroke="#e0d5c0" strokeWidth="1.8" strokeLinecap="round" />
            <line x1="50" y1="50" x2="60" y2="50" stroke="#c62828" strokeWidth="0.8" strokeLinecap="round" />
            <circle cx="50" cy="50" r="2.5" fill="#c9a96e" />
            {[0, 30, 60, 90, 120, 150, 180, 210, 240, 270, 300, 330].map((a) => {
              const outer = 42
              const inner = a % 90 === 0 ? 34 : 38
              const rad = (a * Math.PI) / 180
              return (
                <line
                  key={a}
                  x1={50 + Math.cos(rad) * inner}
                  y1={50 + Math.sin(rad) * inner}
                  x2={50 + Math.cos(rad) * outer}
                  y2={50 + Math.sin(rad) * outer}
                  stroke={a % 90 === 0 ? '#c9a96e' : '#555'}
                  strokeWidth={a % 90 === 0 ? 2 : 0.5}
                />
              )
            })}
            <text x="50" y="30" textAnchor="middle" fill="#c9a96e" fontSize="4" fontFamily="serif" letterSpacing="0.15em">CHRONOHAUS</text>
          </svg>
        </div>
        <div className="featured-info">
          <div className="product-card-brand">{featured.brand}</div>
          <h2 className="section-heading">{featured.name}</h2>
          <div className="featured-price">{formatPrice(featured.price)}</div>
          <p className="featured-desc">{featured.description}</p>
          <div className="featured-actions">
            <button className="btn-gold">View Details</button>
            <button
              className="btn-whatsapp"
              onClick={() => onEnquire?.(featured)}
            >
              <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor">
                <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/>
              </svg>
              Enquire on WhatsApp
            </button>
          </div>
        </div>
      </div>
    </section>
  )
}
